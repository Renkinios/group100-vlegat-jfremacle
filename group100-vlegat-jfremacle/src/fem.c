...
femProblem *femElasticityCreate(femGeo* theGeometry,char const *filename)
{
    ...
    ErrorScan(fscanf(file, "Young modulus : %1.7e \n", &E));
    ErrorScan(fscanf(file, "Poisson ratio : %14.7e \n", &theProblem->nu));
    ErrorScan(fscanf(file, "Mass density : %14.7e \n", &theProblem->rho));
    ErrorScan(fscanf(file, "Gravity : %14.7e \n", &theProblem->g));
    fclose(file);
    printf("iCase=%s\n",iCase);
    printf("E=%f\n",E);
    printf("theProblem->E=%f\n",theProblem->E);
    double nu = theProblem->nu;
    double rho = theProblem->rho;
    double g = theProblem->g;
    femElasticityPrint(theProblem);
    if (strncmp(iCase, "planar strains", strlen("planar strains")) == 0) {
